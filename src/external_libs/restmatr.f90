...
subroutine copy_mm(x_len, y_len, &
    ...
    t_matr(t_x_start:t_x_start+x_len-1, t_y_start:t_y_start+y_len-1) =  &
    f_matr(f_x_start:f_x_start+x_len-1, f_y_start:f_y_start+y_len-1)
...
subroutine copy_mr(x1_len, x2_len, &
    ...
    if (mod .eq. 0) then
          t_ri(t_x1_start:t_x1_start+x1_len-1, t_x2_start:t_x2_start+x2_len-1, t_x3) =  &
        f_matr(f_x1_start:f_x1_start+x1_len-1, f_x2_start:f_x2_start+x2_len-1)
    else if (mod .eq. 1) then
          t_ri(t_x1_start:t_x1_start+x1_len-1, t_x3, t_x2_start:t_x2_start+x2_len-1) =  &
        f_matr(f_x1_start:f_x1_start+x1_len-1, f_x2_start:f_x2_start+x2_len-1)
    else if (mod .eq. 2) then
          t_ri(t_x3, t_x1_start:t_x1_start+x1_len-1, t_x2_start:t_x2_start+x2_len-1) = &
        f_matr(f_x1_start:f_x1_start+x1_len-1, f_x2_start:f_x2_start+x2_len-1)
    endif
